...
- 📍 **快速定位**：点击搜索结果直接跳转到文档内关键词位置，支持"上一处/下一处"切换
- 🗑️ **文档管理**：支持删除文档
...
或直接打开文件：
```
/Users/zhijie/Documents/医学知识库/frontend/index.html
```
（前端通过 fetch API 与后端通信，需要启动后端服务）
...
| 方法 | 路径 | 描述 |
|------|------|------|
| `GET` | `/api/documents` | 获取文档列表（?format=all/docx/rtf/txt&sort=desc/asc） |
...
> **注意**：.docx 文件本质上是一个 ZIP 压缩包，解析器直接读取其中的 XML 结构，提取格式信息并转换为 HTML 内联样式，无需任何外部库。
